...
static uint16_t VCP_Init(void)
{
  ...
  */
	...
	USART_InitTypeDef USART_InitStructure;
 	...
 	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB,ENABLE); /* ??GPIO?? */
	RCC_APB2PeriphClockCmd(RCC_APB1Periph_USART3,ENABLE);/* ??USART?? */

	//??1????????
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource10,GPIO_AF_USART3); //GPIOB10???USART1
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource11,GPIO_AF_USART3); //GPIOB11???USART1
...
static uint16_t VCP_DataTx (void)
{
  ...
  {
    APP_Rx_Buffer[APP_Rx_ptr_in] = USART_ReceiveData(USART3) & 0x7F;
  }
