...
*	修改记录 :
*		版本号  日期        作者     说明
*		V1.0                9527     bsp_DelayMS() 函数需要注意g_RunTime 接近0x7FFFFFFF时的情况
*       
*
*********************************************************************************************************
*/
...
/* 定于软件定时器结构体变量 */
static SOFT_TMR_S g_Tmr[TMR_COUNT];

static void bsp_SoftTimerDec(SOFT_TMR_S *_tmr);
...
{
	g_RunTime++;
	if (g_RunTime == 0x7FFFFFFF)	/* 这个变量是 int32_t 类型，最大数为 0x7FFFFFFF */
	{
		g_RunTime = 0;
	}
}
...
/*******************************************************************************
*	函 数 名: SysTick_ISR
*	功能说明: SysTick中断服务程序，每隔1ms进入1次,在stm32f4xx_it.c 中断服务函数调用
*	形    参:  无
*	返 回 值: 无
*******************************************************************************/
...
int32_t bsp_GetRunTime(void)
{
	...
	runtime = g_RunTime;	/* 这个变量在Systick中断中被改写，因此需要关中断进行保护 */
...
int32_t bsp_CheckRunTime(int32_t _LastTime)
{
	...
	now_time = g_RunTime;	/* 这个变量在Systick中断中被改写，因此需要关中断进行保护 */
