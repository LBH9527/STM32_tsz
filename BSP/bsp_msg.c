...
{
	g_sMsg.Buf[g_sMsg.Write].MsgCode = _MsgCode;
	g_sMsg.Buf[g_sMsg.Write].MsgParam = _MsgParam;

	if (++g_sMsg.Write  >= MSG_FIFO_SIZE)
	{
		g_sMsg.Write = 0;
	}
}
...
uint8_t bsp_GetMsg(MSG_S *_pMsg)
{
	...
	if (g_sMsg.Read == g_sMsg.Write)
	{
		return 0;
	}
	else
	{
		p = &g_sMsg.Buf[g_sMsg.Read];

		if (++g_sMsg.Read >= MSG_FIFO_SIZE)
		{
			g_sMsg.Read = 0;
		}
...
uint8_t bsp_GetMsg2(MSG_S *_pMsg)
{
	...
	if (g_sMsg.Read2 == g_sMsg.Write)
	{
		return 0;
	}
	else
	{
		p = &g_sMsg.Buf[g_sMsg.Read2];

		if (++g_sMsg.Read2 >= MSG_FIFO_SIZE)
		{
			g_sMsg.Read2 = 0;
		}
